...
async function boxdNew(name: string): Promise<void> {
  const r = await runCmd(["boxd", "machine", "new", name, "--json"], { timeoutMs: 3 * 60 * 1000 });
  if (r.code !== 0) throw new Error(`boxd machine new failed: ${r.stderr || r.stdout}`);
}

async function boxdExec(name: string, script: string, envVars: Record<string, string> = {}): Promise<string> {
...
export async function runJob(jobId: string): Promise<void> {
  ...
  try {
    ...
    const tree = await boxdExec(machineName, `cd repo && git ls-files | head -300`);
