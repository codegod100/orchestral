...
// Migrate existing databases that pre-date the atproto_did column
try { db.exec(`ALTER TABLE agents ADD COLUMN atproto_did TEXT`); } catch { /* already exists */ }
...
export function insertAgent(a: Omit<AgentRecord, "created_at">): AgentRecord {
  db.prepare(`
    INSERT INTO agents (id, card_url, name, description, icon_url, agent_card_json, security_type, oidc_issuer, oidc_client_id, oidc_client_secret, access_token, refresh_token, token_expires_at, auth_state, atproto_did)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(
    ...
    a.access_token, a.refresh_token, a.token_expires_at, a.auth_state, a.atproto_did ?? null
  );
...
export function updateAgent(id: string, updates: Partial<AgentRecord>): AgentRecord | null {
  ...
  db.prepare(`
    UPDATE agents SET
      ...
      access_token = ?, refresh_token = ?, token_expires_at = ?, auth_state = ?, atproto_did = ?
    WHERE id = ?
  `).run(
